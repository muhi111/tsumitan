...
import { updateProfile, GoogleAuthProvider, linkWithCredential, linkWithPopup, EmailAuthProvider } from 'firebase/auth';
import { useAtomValue, useSetAtom } from 'jotai';
import { auth } from '../firebase/config';
import { authUserAtom, type AuthUser } from '../atoms';
...
const SignUpForm: React.FC<SignUpFormProps> = ({ onSuccess }) => {
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      console.log('匿名アカウントがGoogleアカウントに正常にリンクされました:', user);
      ...
      switch (error.code) {
        case 'auth/credential-already-in-use':
          errorMessage = 'このGoogleアカウントは既に別のアカウントで使用されています';
          break;
